class Comment
  ...
  # Callbacks.
  after_create :auto_upvote, :set_path, :increment_recipe_comment_count
  ...
  def self.threaded_with_field(recipe, field_name='votes')
    comments = all(:conditions => {:recipe_id => recipe.id}, :order => "path asc, #{field_name} desc")
    results, map  = [], {}
  ...
  def increment_recipe_comment_count
    Recipe.collection.update({"_id" => self.recipe_id}, {"$inc" => {"comment_count" => 1}})
  end
  ...
  def set_path
    unless self.parent_id.blank?
      ...
      self.recipe_id = parent.recipe_id
